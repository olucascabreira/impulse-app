...
export function RecurringTransactionEditForm({
  ...
  const { updateRecurringTransaction } = useRecurringTransactions(recurringTransaction.id);
  const { toast } = useToast();
  const { bankAccounts } = useBankAccounts(recurringTransaction.id);
  const { chartAccounts } = useChartAccounts(recurringTransaction.id);
  const { contacts } = useContacts(recurringTransaction.id);
  ...
  const form = useForm<RecurringTransactionFormData>({
    ...
    defaultValues: {
      ...
      amount: formatCurrency(recurringTransaction.amount).replace('R$ ', ''),
      ...
      recurrence_type: recurringTransaction.recurrence_type,
  ...
  const onSubmit = async (data: RecurringTransactionFormData) => {
    try {
      ...
      const result = await updateRecurringTransaction(recurringTransaction.id, recurringTransactionData);
      ...
      toast({
        ...
        description: "A transação recorrente foi atualizada com sucesso.",
      });
      ...
      toast({
        ...
        description: error.message || "Ocorreu um erro ao atualizar a transação recorrente",
        variant: "destructive",
      });
    }
  };
    ...
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
        ...
        <div className="flex justify-end space-x-2 pt-4">
          ...
          <Button type="submit">Atualizar Transação Recorrente</Button>
        </div>
      </form>
    </Form>
  );
